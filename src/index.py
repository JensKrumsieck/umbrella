...
def read_coverage(user: str, repo: str, branch="main"):
    try:
        ...
        if not response.ok:
            raise HTTPException(status_code=response.status_code, detail=f"GitHub API returned error. Request URL: {zip_url}")
        ...
        with zipfile.ZipFile(zip_file, 'r') as zip_ref:
            ...
            if xml_file_name:
                ...
                raise HTTPException(status_code=500, detail="could not find the required xml file...")
        ...
        return HTTPException(status_code=500, detail="🚨 Oh no! I could not proceed on your request...")
